...
export default function EditTransactionScreen({ route, navigation }) {
  ...
  const handleUpdate = async () => {
    if (!betrag || parseFloat(betrag) <= 0) {
      Alert.alert(t('common.error'), t('edit.invalidAmount'));
      return;
    }
    ...
    Alert.alert(t('common.success'), t('edit.saved'));
    navigation.goBack();
  };
  ...
  if (!originalTransaction) {
    return (
      <View style={[styles.container, { backgroundColor: currentTheme.background, justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: currentTheme.text }}>{t('edit.loading')}</Text>
      </View>
    );
  }
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={[styles.container, { backgroundColor: currentTheme.background }]}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={[styles.formCard, { backgroundColor: currentTheme.cardBackground }]}>
          <Text style={[styles.formTitle, { color: currentTheme.text }]}>{t('edit.title')}</Text>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.kategorienContainer}>
              {KONTEN.map((k) => (
                <TouchableOpacity
                  key={k.name}
                  style={[styles.kategorieButton, { backgroundColor: konto === k.name ? currentTheme.primary : currentTheme.surface, borderColor: konto === k.name ? currentTheme.primary : currentTheme.border }]}
                  onPress={() => setKonto(k.name)}
                >
                  <Ionicons name={k.icon} size={24} color={konto === k.name ? '#fff' : currentTheme.primary} />
                  <Text style={[styles.kategorieText, { color: konto === k.name ? '#fff' : currentTheme.text }]}>{tName(k.name)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.kategorienContainer}>
              {KATEGORIEN.map((kat) => (
                <TouchableOpacity
                  key={kat.name}
                  style={[styles.kategorieButton, { backgroundColor: kategorie === kat.name ? currentTheme.primary : currentTheme.surface, borderColor: kategorie === kat.name ? currentTheme.primary : currentTheme.border }]}
                  onPress={() => setKategorie(kat.name)}
                >
                  <Ionicons name={kat.icon} size={24} color={kategorie === kat.name ? '#fff' : kat.color} />
                  <Text style={[styles.kategorieText, { color: kategorie === kat.name ? '#fff' : currentTheme.text }]}>{tName(kat.name)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
          ...
          <TouchableOpacity style={[styles.speichernButton, { backgroundColor: currentTheme.primary }]} onPress={handleUpdate}>
            ...
            <Text style={styles.speichernButtonText}>{t('edit.saveButton')}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
